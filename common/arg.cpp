...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--moe-expert-cache-window"}, "N",
        string_format("MoE expert cache admission window in tokens (default: %d)", params.n_moe_cache_window),
        [](common_params & params, int value) {
            params.n_moe_cache_window = value;
        }
    ).set_env("LLAMA_ARG_MOE_EXPERT_CACHE_WINDOW"));
    add_opt(common_arg(
        {"--moe-expert-cache-admit"}, "N",
        string_format("misses within the window before an expert is uploaded to the MoE expert cache, 1 = ungated (default: %d)", params.n_moe_cache_admit),
        [](common_params & params, int value) {
            params.n_moe_cache_admit = value;
        }
    ).set_env("LLAMA_ARG_MOE_EXPERT_CACHE_ADMIT"));
    add_opt(common_arg(
        {"--moe-expert-cache-warm"}, "N",
        string_format("prompt batches of at least N tokens re-rank the MoE expert cache slots by their routing frequency, 0 = disabled (default: %d)", params.n_moe_cache_warm),
        [](common_params & params, int value) {
